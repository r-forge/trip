...
subset.trip <- function(x,  ...) {
    ## spdf <- subset(as(x, "SpatialPointsDataFrame"), subset,
    ##                select, drop=drop, ...)
...
print.trip <- function(x, ...) {
    ...
    nms <- names(x)
    ## names(nms) <- names(x)
    ## nms[[xs[["TORnames"]][1]]] <- paste(nms[[xs[["TORnames"]][1]]],
    ##                                     "*trip DateTime*")
    ## nms[[xs[["TORnames"]][2]]] <- paste(nms[[xs[["TORnames"]][2]]],
    ##                                     "#trip ID#")
    clss <- unlist(lapply(as.data.frame(x@data),  function(x) class(x)[1]))
    ## names(clss) <- names(x)
    ## clss[[xs[["TORnames"]][1]]] <- paste(clss[[xs[["TORnames"]][1]]],
    ##                                      "*trip DateTime*")
    ## clss[[xs[["TORnames"]][2]]] <- paste(clss[[xs[["TORnames"]][2]]],
    ##                                      "#trip ID#")
